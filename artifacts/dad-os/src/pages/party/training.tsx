...
export default function PartyTraining() {
      ...
      <div className="p-6 md:p-10 max-w-4xl mx-auto space-y-8">
        <header className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-4xl font-serif font-bold tracking-tight text-foreground flex items-center gap-3">
              <Swords className="w-8 h-8 text-primary" /> Training Sessions
            </h1>
            
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button className="rounded-full font-bold shadow-sm" data-testid="btn-add-training">
                  <Plus className="mr-2 h-4 w-4" /> Add Session
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Plan Training Session</DialogTitle>
                </DialogHeader>
                <Form {...form}>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input placeholder="e.g. Build a fort" {...field} data-testid="input-training-title" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Textarea placeholder="Details about the session..." {...field} data-testid="input-training-desc" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    <Button type="submit" className="w-full font-bold" disabled={createActivity.isPending} data-testid="btn-submit-training">
                      {createActivity.isPending ? "Planning..." : "Plan Session"}
                    </Button>
                  </form>
                </Form>
              </DialogContent>
        ...
        {isLoading ? (
           ...
           <div className="p-12 text-center border-2 border-dashed rounded-xl">
             ...
             <h3 className="text-xl font-bold">No Training Sessions</h3>
             <p className="text-muted-foreground mt-2">Plan a session to earn XP.</p>
           </div>
        ) : (
           <div className="grid gap-4">
              {activities?.map(activity => (
                 <Card key={activity.id} className={`transition-all border-2 ${activity.completed ? 'opacity-70 bg-secondary/20' : 'hover-elevate shadow-sm'}`}>
                   <CardContent className="p-5 flex items-start gap-4">
                      ...
                      <div className="flex-1">
                         <div className="flex items-center gap-2 mb-1">
                            <h3 className={`text-xl font-bold ${activity.completed ? 'line-through text-muted-foreground' : 'text-foreground'}`}>{activity.title}</h3>
                            <Badge variant={activity.completed ? 'outline' : 'default'} className="ml-2 uppercase tracking-wider text-xs">{activity.category}</Badge>
                         </div>
                         {activity.description && <p className="text-muted-foreground font-medium">{activity.description}</p>}
                      </div>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(activity.id)} className="text-destructive/50 hover:text-destructive hover:bg-destructive/10" data-testid={`btn-del-act-${activity.id}`}>
