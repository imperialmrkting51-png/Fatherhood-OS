...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { label: "Training", href: "/training", icon: Compass },
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-14 items-center justify-between">
          <div className="flex items-center gap-4">
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              ...
              <SheetContent side="left" className="w-[280px] p-6 bg-card border-r-0">
                <div className="flex items-center gap-2 mb-8">
                  <span className="font-serif font-bold text-2xl text-primary">Fatherhood OS</span>
                </div>
                <NavLinks />
              </SheetContent>
            </Sheet>
            <Link href="/">
               <div className="flex items-center gap-2 cursor-pointer">
                 <span className="font-serif font-bold text-xl hidden md:inline-block text-primary">Fatherhood OS</span>
               </div>
            </Link>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-primary/10 text-primary px-3 py-1.5 rounded-full font-bold">
              Lvl {gameState.level}
            </div>
          </div>
        </div>
        <div className="w-full bg-secondary/30 border-t py-2 px-4 md:px-8">
          <div className="container flex items-center gap-4">
            <span className="text-xs font-semibold text-muted-foreground w-24 text-right shrink-0">Level {gameState.level}</span>
            <Progress value={gameState.xp} className="h-2 flex-1" data-testid="xp-bar" />
            <span className="text-xs font-semibold text-muted-foreground w-24 shrink-0">{gameState.xp} / 100 XP</span>
          </div>
        </div>
      </header>
